...
$(document).ready(function () {
    // INIT CHART - use setTimeout to ensure DOM is ready
    setTimeout(function() {
        var c = document.getElementById("myChart");
        if(c && c.getContext) {
            window.myChart = new Chart(c.getContext("2d"), {
                type: "bar",
                data: {
                    labels: ["IP"],
                    datasets: [{ label: "Count", data: [1], backgroundColor: "#4CAF50" }]
                }
            });
    ...
    var firebaseConfig = {
        apiKey: "AIzaSyDEMO_KEY",
        authDomain: "nerrofence.firebaseapp.com",
        projectId: "nerrofence",
        storageBucket: "nerrofence.appspot.com",
    ...
    // Initialize Socket.IO with polling instead
...
if(data && data.result) {
                    ...
                    if(table) {
                        ...
                        row.innerHTML = "<td>" + data.result[0] + "</td><td>" + data.result[1] + "</td><td>" + data.result[2] + "</td><td>" + data.result[3] + "</td><td>" + data.result[4] + "</td><td>" + data.result[5] + "</td><td>" + data.result[6] + "</td><td>" + data.result[7] + "</td><td></td>";
                    }
                    
                    // Update chart
                    if(data.ips && data.ips.length > 0) {
                        var labels = data.ips.map(function(x){return x.SourceIP});
                        var counts = data.ips.map(function(x){return x.count});
                        if(window.myChart) {
                    ...
                    messages_received.push(data.result);
                    updateTable(currentPage);
                    saveFlowsToLocal();
                    
                    // Update chart
                    console.log('Updating chart with data:', data.ips);
                    try {
                        var chartLabels = data.ips.map(function(x){return x.SourceIP});
                        var chartData = data.ips.map(function(x){return x.count});
                        console.log('Chart labels:', chartLabels, 'data:', chartData);
                        console.log('myChart exists:', typeof window.myChart);
                        if(typeof window.myChart !== 'undefined') {
                            window.myChart.data.labels = chartLabels;
                            window.myChart.data.datasets[0].data = chartData;
                            window.window.myChart.update();
                            console.log('Chart updated!');
                        } else {
                            console.log('myChart not ready');
                        }
                    } catch(e) {
...
auth.onAuthStateChanged(user => {
    ...
    // Chart already initialized at start
        type: 'bar',
        data: {
            labels: [],
            datasets: [{
